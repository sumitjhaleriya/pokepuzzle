...
def ShowEndInterface(screen, width, height):
    ...
    title = font.render('Congrates! You Won!', True, (233, 150, 122))
    rect = title.get_rect()
...
def ShowStartInterface(screen, width, height):
    screen.fill(cfg.BACKGROUNDCOLOR)
    tfont = pygame.font.Font(cfg.FONTPATH, width//5)
    cfont = pygame.font.Font(cfg.FONTPATH, width//24)
    title = tfont.render('PokePuzzle',True, cfg.RED)
    content1 = cfont.render('Press H or M or L to choose your Puzzle', True, cfg.BLUE)
    content2 = cfont.render('H-5*5,M-4*4,L-3*3', True, cfg.BLUE)
    content3 = cfont.render('Can also use h=left,j=up,k=down,l=right',True, cfg.RED)
    trect = title.get_rect()
    trect.midtop = (width/2, height/9)
    crect1 = content1.get_rect()
    crect1.midtop = (width/2, height/2.5)
    crect2 = content2.get_rect()
    crect2.midtop = (width/2, height/1.5)
    crect3 = content3.get_rect()
    crect3.midtop = (width/1.9, height/1.2)
    ...
    while True:
        for event in pygame.event.get():
            if (event.type == pygame.QUIT) or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                ...
                if event.key == ord('a'): return 3
                elif event.key == ord('s'): return 4
                elif event.key == ord('d'): return 5
        pygame.display.update()
    ...
    while is_running:
        ...
        for event in pygame.event.get():
            ...
            if (event.type == pygame.QUIT) or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                ...
                elif event.key == pygame.K_RIGHT or event.key == ord('l'):
                    blank_cell_idx = moveR(game_board, blank_cell_idx, num_cols)
                elif event.key == pygame.K_UP or event.key == ord('j'):
                    blank_cell_idx = moveU(game_board, blank_cell_idx, num_rows, num_cols)
                elif event.key == pygame.K_DOWN or event.key == ord('k'):
